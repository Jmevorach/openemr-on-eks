...
#   3. Installs Warp
#   4. Imports 1000 users from the test dataset
#   5. Prints out login address and credentials
#
# Usage:
#   ./scripts/test-warp-end-to-end.sh [OPTIONS]
#
# Options:
...
CLUSTER_NAME=""
DATA_SOURCE="${DATA_SOURCE:-s3://synpuf-omop/cmsdesynpuf1k/}"
    ...
    spec:
      ...
      - name: warp
        ...
        - |
          set -e
          echo "=========================================="
          echo "Warp End-to-End Test Import"
          echo "=========================================="
...
EOF
    ...
    if kubectl wait --for=condition=complete --timeout=1800s job/warp-e2e-test-import -n "$NAMESPACE" >/dev/null 2>&1; then
        log_success "Data import completed successfully"
        
        # Show job logs
        log_step "Import job logs:"
        kubectl logs -n "$NAMESPACE" job/warp-e2e-test-import --tail=50
    else
        log_error "Job failed or timed out"
        log_info "Job status:"
        kubectl get job -n "$NAMESPACE" warp-e2e-test-import
        log_info "Job logs:"
        kubectl logs -n "$NAMESPACE" job/warp-e2e-test-import --tail=100 || true
        exit 1
    fi
}
...
print_credentials() {
    ...
    echo ""
    echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${GREEN}🎉 Warp End-to-End Test Complete!${NC}"
    echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    ...
    echo "  4. Check import statistics in the Warp job logs"
    echo ""
    echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
}
...
wait_for_verification() {
    log_header "Step 6: Verification Period"
    
    log_info "Waiting 5 minutes for user verification..."
    log_info "You can now log in and verify the deployment is working correctly."
    log_info "The test will automatically proceed with cleanup after 5 minutes."
    echo ""
    
    # Countdown timer
    local remaining=300  # 5 minutes = 300 seconds
    while [ $remaining -gt 0 ]; do
        local minutes=$((remaining / 60))
        local seconds=$((remaining % 60))
        printf "\r${CYAN}⏱️  Time remaining: %02d:%02d${NC}" "$minutes" "$seconds"
        sleep 1
    ...
    log_success "Verification period complete"
}

run_cleanup() {
    log_header "Step 7: Cleaning Up Infrastructure"
    
    log_step "Running destroy.sh to clean up all infrastructure..."
    log_info "This will destroy all AWS resources created during the test."
    
    cd "$SCRIPT_DIR" || exit 1
    
    # Run destroy.sh with --force to skip prompts
    if ./destroy.sh --force; then
...
print_test_statistics() {
    ...
    echo ""
    echo -e "  ${BLUE}Test Status:${NC}     $TEST_STATUS"
    echo -e "  ${BLUE}Total Duration:${NC}  ${minutes}m ${seconds}s"
    echo -e "  ${BLUE}Records Imported:${NC} $MAX_RECORDS"
    echo -e "  ${BLUE}Data Source:${NC}     $DATA_SOURCE"
    echo -e "  ${BLUE}Cluster Name:${NC}    $CLUSTER_NAME"
    echo -e "  ${BLUE}AWS Region:${NC}      $AWS_REGION"
...
DESCRIPTION:
    ...
    2. Deploys OpenEMR on EKS
    3. Installs Warp
    4. Imports users from the test dataset
    5. Prints out login address and credentials
...
ENVIRONMENT VARIABLES:
    ...
    DATA_SOURCE             S3 path to OMOP data
...
EXAMPLES:
    ...
    # Custom data source
    $0 --data-source s3://my-bucket/my-data/
...
main() {
    ...
    log_header "Warp End-to-End Test Starting"
    ...
    if ! install_warp; then
        log_error "Warp installation failed"
        print_test_statistics
        exit 1
    fi
    ...
    # Wait 5 minutes for user verification
    wait_for_verification
    ...
    if run_cleanup; then
        ...
        TEST_STATUS="FAILED"
        log_error "Cleanup failed, but test deployment was successful"
    fi
