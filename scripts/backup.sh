...
if kubectl cluster-info >/dev/null 2>&1; then
    ...
    if [ -n "$POD_NAME" ]; then
        ...
        if kubectl exec -n "$NAMESPACE" "$POD_NAME" -- tar -czf "/tmp/${APP_BACKUP_FILE}" -C /var/www/localhost/htdocs/openemr sites/ 2>/dev/null; then
            # Copy from pod
            ...
            kubectl exec -n "$NAMESPACE" "$POD_NAME" -- rm -f "/tmp/${APP_BACKUP_FILE}" 2>/dev/null || true
        else
            log_warning "Failed to create application data backup"
            add_result "Application Data" "FAILED" "Backup creation failed"
        fi
    else
...
2. Follow the prompts for infrastructure setup
